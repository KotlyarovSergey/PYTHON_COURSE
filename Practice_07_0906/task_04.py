# 7.4[49]. Напишите функцию same_by(func, objects) которая проверяет, все ли 
# элементы в objects дают одинаковое значение характеристики func 
# Аргументы:
...
def same_by(func, objects):
    ...
    result = func(objects[0])
    for ind in range(1, len(objects)):
        if result != func(objects[ind]):
            return False
    return True
